...
final class EfficientTAMWeights
{
    ...
    func shape(named name: String) throws -> [Int]
    {
        ...
        {
            throw EfficientTAMError("Missing EfficientTAM tensor '\(name)'.")
        }
        return entry.shape
    }
    ...
    func floats(named name: String) throws -> [Float]
    {
        ...
        {
            throw EfficientTAMError("EfficientTAM tensor '\(name)' has unsupported dtype '\(entry.dtype)'.")
        }
        ...
        {
            throw EfficientTAMError("EfficientTAM tensor '\(name)' exceeds the weights file.")
        }
